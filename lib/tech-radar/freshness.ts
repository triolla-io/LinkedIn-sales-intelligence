...
export type FreshnessSpread = { freshest: number | null; median: number | null; oldest: number | null };

/**
 * How old a batch actually is, in whole days. All-null for an empty batch rather than
 * zeros, because zeros would read as "everything is fresh".
 *
 * The 2026-08-26 report said "poolItems: 200, itemsWritten: 11" and every one of the
 * eleven was stale — the freshest by 56 days. Nothing in the report could show it: the
 * report carried counts and no age. The counts alone cannot answer "is this week's radar
 * actually about this week".
 */
export function freshnessSpread<T extends { publishedAt?: string | null }>(
  items: T[],
  now: Date
): FreshnessSpread {
  const ages: number[] = [];
  for (const i of items) {
    const ms = publishedMs(i.publishedAt);
    if (ms === 0) continue;
    ages.push(Math.max(0, Math.floor((now.getTime() - ms) / DAY_MS)));
  }
  if (ages.length === 0) return { freshest: null, median: null, oldest: null };
  ages.sort((a, b) => a - b);
  return { freshest: ages[0], median: ages[Math.floor((ages.length - 1) / 2)], oldest: ages[ages.length - 1] };
}
