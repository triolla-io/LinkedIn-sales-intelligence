...
describe("buildSearchUrl", () => {
  it("encodes keywords and pins 2nd-degree-only + Israel filters", () => {
    const url = new URL(buildSearchUrl({ keywords: "cto, vp r&d, ceo" }, 1));
  ...
  it("includes the page param only when page > 1", () => {
    expect(new URL(buildSearchUrl({ keywords: "ceo" }, 1)).searchParams.has("page")).toBe(false);
    expect(new URL(buildSearchUrl({ keywords: "ceo" }, 3)).searchParams.get("page")).toBe("3");
  });
});
