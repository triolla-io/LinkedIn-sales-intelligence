import{a as $,h as k,i as D,p as O,r as M}from"./api-IMK_rRYA.js";const P="1.3",m="automationWindowId";async function B(e){try{return await chrome.windows.get(e),!0}catch{return!1}}async function L(){const t=(await chrome.storage.local.get(m))[m];if(t!==void 0&&await B(t))return t;t!==void 0&&await chrome.windows.remove(t).catch(()=>{});const n=await chrome.windows.create({focused:!1,state:"minimized"});if(!(n!=null&&n.id))throw new Error("automation_window_create_failed");return await chrome.storage.local.set({[m]:n.id}),n.id}async function _(e,t=!0){const n=await L(),a=await chrome.tabs.create({windowId:n,url:e,active:t});if(!a.id)throw new Error("tab_create_failed");return await chrome.windows.update(n,{focused:!1,state:"minimized"}).catch(()=>{}),a.id}async function q(){const t=(await chrome.storage.local.get(m))[m];t!==void 0&&!await B(t)&&await chrome.storage.local.remove(m)}async function E(e){await new Promise((t,n)=>{chrome.debugger.attach({tabId:e},P,()=>{chrome.runtime.lastError?n(new Error(chrome.runtime.lastError.message)):t()})})}async function S(e){await new Promise(t=>{chrome.debugger.detach({tabId:e},()=>{chrome.runtime.lastError,t()})})}async function s(e,t,n={}){return await new Promise((a,i)=>{chrome.debugger.sendCommand({tabId:e},t,n,o=>{chrome.runtime.lastError?i(new Error(chrome.runtime.lastError.message)):a(o)})})}async function g(e,t,n){await s(e,"Input.dispatchMouseEvent",{type:"mouseMoved",x:t,y:n,button:"none",buttons:0}),await b(50),await s(e,"Input.dispatchMouseEvent",{type:"mousePressed",x:t,y:n,button:"left",buttons:1,clickCount:1}),await b(50),await s(e,"Input.dispatchMouseEvent",{type:"mouseReleased",x:t,y:n,button:"left",buttons:0,clickCount:1})}async function U(e){var n;const t=await s(e,"Runtime.evaluate",{expression:`(function() {
  ...
  return 'connectable';
})()`,X=30,j=60,x="0.3.2";let v=!1;async function A(e){await chrome.storage.local.set({swActiveTabId:e})}async function y(){await chrome.storage.local.remove("swActiveTabId")}(async()=>{try{await q().catch(()=>{});const{swActiveTabId:e}=await chrome.storage.local.get("swActiveTabId");e&&(console.log("[startup] closing orphaned tab",e),await chrome.tabs.remove(e).catch(()=>{}),await y())}catch(e){console.warn("[startup] cleanup error",e)}})();chrome.runtime.onInstalled.addListener(()=>{chrome.alarms.create("poll",{periodInMinutes:X/60}),chrome.alarms.create("hb",{periodInMinutes:j/60})});$().then(e=>{e&&k(x)});chrome.runtime.onMessage.addListener(e=>{(e==null?void 0:e.type)==="heartbeat"&&k(x)});chrome.alarms.onAlarm.addListener(async e=>{if(await $()&&!await D()){if(e.name==="hb"){await k(x);return}if(e.name==="poll"){if(v){console.log("[poll] task already running, skipping");return}for(;await G(););}}});async function G(){let e;try{e=await O()}catch(t){return console.warn("poll error",t),!1}if(!e)return!1;v=!0;try{const t=await Q(e);await M(e.id,{ok:!0,result:t})}catch(t){const n=t.code??"unknown",a=t.screenshot,i=t.buttons,o=t.diag;await M(e.id,{ok:!1,errorCode:n,errorMessage:t.message,...a||i||o?{result:{debugScreenshot:a,buttons:i,diag:o}}:{}})}finally{v=!1}return!0}async function Q(e){const t=e.payload;if(e.kind==="SEND"){if(!t.linkedinUrl||!t.text)throw h(new Error("missing_payload"),"bad_payload");return await tt(t.linkedinUrl,t.text,t.recipientName??"")}if(e.kind==="CHECK_REPLY")return{replyDetected:!1,replies:[]};if(e.kind==="SEARCH"){if(!t.searchUrl)throw h(new Error("missing_payload"),"bad_payload");return await nt(t.searchUrl)}if(e.kind==="CONNECT"){if(!t.profileUrl)throw h(new Error("missing_payload"),"bad_payload");return await ot(t.profileUrl)}throw h(new Error("unknown_kind"),"bad_payload")}async function Z(e){var n;const t={};try{const a=await chrome.tabs.get(e);t.tabUrl=a.url??null,t.tabStatus=a.status??null,t.tabTitle=a.title??null,t.windowId=a.windowId??null}catch(a){t.tabGetError=String((a==null?void 0:a.message)??a)}try{if((n=chrome.management)!=null&&n.getAll){const a=await chrome.management.getAll();t.extensions=a.filter(i=>i.type==="extension").map(i=>({id:i.id,name:i.name,enabled:i.enabled}))}else t.extensions="management_api_unavailable"}catch(a){t.managementError=String((a==null?void 0:a.message)??a)}return t}async function tt(e,t,n=""){const a=await _("about:blank",!1);await A(a);let i=!1,o=null;try{await p(a),await E(a),i=!0,await s(a,"Emulation.setDeviceMetricsOverride",{width:1280,height:900,deviceScaleFactor:1,mobile:!1}).catch(()=>{}),await s(a,"Page.navigate",{url:e}),await p(a),await f(2500);const l=await chrome.tabs.get(a);if(l.url&&l.url.includes("/checkpoint"))throw h(new Error("checkpoint"),"checkpoint");await z(a),await f(500),await H(a)&&(console.log("[agent] dismissed popup before Message click"),await f(500));const c=await U(a);if(!c)throw h(new Error("message_button_not_found"),"not_messageable");console.log("[agent] composeUrl:",c),await chrome.tabs.update(a,{url:c}),await p(a);let r=await T(a);const d=Date.now()+15e3;for(;Date.now()<d&&r.msgForm===0&&r.anyEditable===0;)await f(500),r=await T(a);console.log("[agent] post-nav diag:",r);const R=await V(a,t);if(console.log("[agent] typeIntoCompose:",R),!R)throw h(new Error(`compose_insert_failed diag=${JSON.stringify(r)}`),"compose_insert_failed");await f(600);const C=await F(a);if(console.log("[agent] clickSendButton:",C),!C)throw h(new Error("send_button_not_found"),"send_button_not_found");return await f(1500),{sentAt:new Date().toISOString(),conversationUrl:e,steps:3}}catch(l){o=l;const u=await chrome.tabs.get(a).catch(()=>null);if(u!=null&&u.url&&(o.message=`${o.message} (url=${u.url})`),o.diag=await Z(a).catch(()=>({diagError:!0})),i)try{const[c,r]=await Promise.all([N(a),w(a)]);o.screenshot=c,o.buttons=r}catch{}throw o}finally{i&&await S(a).catch(()=>{}),await chrome.tabs.remove(a).catch(()=>{}),await y()}}const et=`(() => {
  ...
  return { candidates: out, hasNextPage };
})()`;async function nt(e){var a;const t=await _(e).catch(()=>{throw h(new Error("tab_create_failed"),"tab_load")});await A(t);let n=!1;try{await p(t),await f(2500);const i=await chrome.tabs.get(t);if(i.url&&i.url.includes("/checkpoint"))throw h(new Error("checkpoint"),"checkpoint");await E(t),n=!0,await s(t,"Emulation.setDeviceMetricsOverride",{width:1280,height:900,deviceScaleFactor:1,mobile:!1}).catch(()=>{});for(let u=0;u<6;u++)await Y(t,1200),await f(800);const o=await s(t,"Runtime.evaluate",{expression:et,returnByValue:!0}),l=(a=o==null?void 0:o.result)==null?void 0:a.value;if(!l)throw h(new Error("scrape_returned_null"),"scrape_failed");return l}finally{n&&await S(t).catch(()=>{}),await chrome.tabs.remove(t).catch(()=>{}),await y()}}async function I(e,t){var a;const n=await s(e,"Runtime.evaluate",{expression:`(() => {
    ...
    })()`,returnByValue:!0});return((n=t==null?void 0:t.result)==null?void 0:n.value)??null}async function ot(e){var a;const t=await _("about:blank",!1).catch(()=>{throw h(new Error("tab_create_failed"),"tab_load")});await A(t);let n=!1;try{await p(t),await E(t),n=!0,await s(t,"Emulation.setDeviceMetricsOverride",{width:1280,height:900,deviceScaleFactor:1,mobile:!1}).catch(()=>{}),await s(t,"Page.navigate",{url:e}),await p(t),await f(4e3);const i=await chrome.tabs.get(t);if(i.url&&i.url.includes("/checkpoint"))throw h(new Error("checkpoint"),"checkpoint");const o=(e.split("/in/")[1]??"").replace(/[/?#].*/,"").toLowerCase();let l=await I(t,o);if(console.log("[connect] directBtn:",l),!l){let c=await w(t);if(console.log("[connect] buttons found:",c.map(r=>`"${r.text}" aria="${r.aria}" y=${r.y}`)),l=c.find(r=>/^connect$/i.test(r.text)||/connect/i.test(r.aria))??null,!l){const r=c.find(d=>/^more$/i.test(d.text)||/^more$/i.test(d.aria));r&&(await g(t,r.x+Math.round(r.w/2),r.y+Math.round(r.h/2)),await f(700),c=await w(t),console.log("[connect] buttons after More:",c.map(d=>`"${d.text}" aria="${d.aria}" y=${d.y}`)),l=c.find(d=>/^connect$/i.test(d.text)||/connect/i.test(d.aria))??null,l||(l=await I(t,o)))}}if(!l){const c=await s(t,"Runtime.evaluate",{expression:J,returnByValue:!0}),r=(a=c==null?void 0:c.result)==null?void 0:a.value;throw r==="pending"?h(new Error("invitation_already_pending"),"already_pending"):r==="connected"?h(new Error("already_connected"),"already_connected"):h(new Error("connect_button_not_found"),"no_connect")}await g(t,l.x+Math.round((l.w??80)/2),l.y+Math.round((l.h??36)/2));let u=null;for(let c=0;c<6&&(await f(c===0?1500:800),u=await at(t),!u);c++);if(console.log("[connect] sendBtn:",u),!u){const c=await w(t);console.log("[connect] afterButtons:",c.map(d=>`"${d.text}" aria="${d.aria}"`));const r=c.map(d=>(d.text||d.aria||"").trim()).filter(Boolean).slice(0,12).join(" | ");throw h(new Error(`send_dialog_not_found; buttons=[${r}]`),"already_or_blocked")}return await g(t,u.x+Math.round(u.w/2),u.y+Math.round(u.h/2)),await f(800),{sentAt:new Date().toISOString()}}catch(i){const o=i,l=await chrome.tabs.get(t).catch(()=>null);if(l!=null&&l.url&&(o.message=`${o.message} (url=${l.url})`),n&&o.code!=="already_pending"&&o.code!=="already_connected"&&o.code!=="checkpoint")try{const[u,c]=await Promise.all([N(t),w(t)]);o.screenshot=u,o.buttons=c}catch{}throw o}finally{n&&await S(t).catch(()=>{}),await chrome.tabs.remove(t).catch(()=>{}),await y()}}async function p(e){const t=await chrome.tabs.get(e).catch(()=>null);(t==null?void 0:t.status)!=="complete"&&await new Promise((n,a)=>{let i=!1;const o=r=>{i||(i=!0,clearTimeout(l),clearInterval(c),chrome.tabs.onUpdated.removeListener(u),r())},l=setTimeout(()=>o(()=>a(h(new Error("tab_load_timeout"),"tab_load"))),3e4),u=(r,d)=>{r===e&&d.status==="complete"&&o(n)};chrome.tabs.onUpdated.addListener(u);const c=setInterval(async()=>{const r=await chrome.tabs.get(e).catch(()=>null);if(!r)return o(()=>a(h(new Error("tab_closed"),"tab_load")));r.status==="complete"&&o(n)},1e3)})}function f(e){return new Promise(t=>setTimeout(t,e))}function h(e,t){return e.code=t,e}
